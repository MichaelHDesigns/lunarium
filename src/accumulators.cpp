...
uint32_t ParseChecksum(uint256 nChecksum, CoinDenomination denomination)
{
    ...
    int pos = boost::iterators::distance(zerocoinDenomList.begin(), find(zerocoinDenomList.begin(), zerocoinDenomList.end(), denomination));
...
bool ValidateAccumulatorCheckpoint(const CBlock& block, CBlockIndex* pindex, AccumulatorMap& mapAccumulators)
{
    ...
    //It is VERY IMPORTANT that when this is being run and height < v2_start, then zXLN need to be disabled at the same time!!
