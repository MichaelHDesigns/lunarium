...
bool CMasternodeConfig::read(std::string& strErr)
{
    ...
    if (!streamConfig.good()) {
        ...
        if (configFile != NULL) {
            std::string strHeader = "# Masternode config file\n"
                                    ...
                                    "# Example: mn1 127.0.0.2:44071 93HaYBVUCYjEMeeH1Y4sBGLALQZE1Yc1K64xiqgX37tGBDQL8Xg 2bcd3c84c84f87eaa86e4e56834c92927a07f9e18718810b92e0d0324456a67c 0\n";
    ...
    for (std::string line; std::getline(streamConfig, line); linenumber++) {
        ...
        if (Params().NetworkID() == CBaseChainParams::MAIN) {
            if (CService(ip).GetPort() != 44071) {
                strErr = _("Invalid port detected in masternode.conf") + "\n" +
                         strprintf(_("Line: %d"), linenumber) + "\n\"" + line + "\"" + "\n" +
                         _("(must be 44071 for mainnet)");
                streamConfig.close();
                return false;
            }
        } else if (CService(ip).GetPort() == 44071) {
            strErr = _("Invalid port detected in masternode.conf") + "\n" +
                     strprintf(_("Line: %d"), linenumber) + "\n\"" + line + "\"" + "\n" +
                     _("(44071 could be used only on mainnet)");
            streamConfig.close();
            return false;
        }
...
void CMasternodeConfig::writeToMasternodeConf()
{
    ...
    // add file header back as each time it restarts the file
    std::string strHeader = "# Masternode config file\n"
                            "# Format: alias IP:port masternodeprivkey collateral_output_txid collateral_output_index\n"
                            "# Example: mn1 127.0.0.2:44071 93HaYBVUCYjEMeeH1Y4sBGLALQZE1Yc1K64xiqgX37tGBDQL8Xg 2bcd3c84c84f87eaa86e4e56834c92927a07f9e18718810b92e0d0324456a67c 0\n";
    fwrite(strHeader.c_str(), std::strlen(strHeader.c_str()), 1, configFile);
